"""Desktop mentor C: capture a strong desktop coding assistant's answers
as training examples for the local model.

The desktop coding mentor is an AI code editor; its chat panel (Ctrl+L) can create and edit files
in the open project folder. Answers are saved to D:\\hwk-data\\teacher with
source "mentor-c", and any files it writes stay in the project folder so they can
be harvested as code examples later.

Usage:
  python mentor_c.py "Build a small guessing game in Python in this folder"
  python mentor_c.py --file questions.txt
"""
...
def _main_window() -> tuple[int, tuple[int, int, int, int]]:
    ...
    if not candidates:
        raise SystemExit("mentor window not found. Is the mentor app running and signed in?")
...
def main() -> None:
    _utf8_stdio()
    parser = argparse.ArgumentParser(description="Ask the coding mentor and save the answer.")

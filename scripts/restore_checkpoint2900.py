...
def main() -> int:
    ...
    try:
        ...
        if model_id and "checkpoint-2900" not in model_id:
            log("RESTORE FAILED: endpoint is NOT serving checkpoint-2900")
            server.terminate()
            kill_port_owner(sp.PORT)
            return 1

        if args.exam:
            tuned = sp.run_exam("restored-checkpoint2900", sp._model_for_adapter())
            if tuned is None:
    ...
    promoted = {
        ...
        "note": "restored from tuned.checkpoint2900.promoted.bak",
    }
    PROMOTED_JSON.write_text(json.dumps(promoted, indent=2) + "\n", encoding="utf-8")
    log(f"promoted.json updated - checkpoint-2900 is the live brain on :{sp.PORT}")
    return 0

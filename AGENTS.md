...
- **sft_v3 media-mix plan (2026-09-12 night, awaiting owner go)**: tasks/sft-v3-image-tool-mix.md — why every media exam case failed (0/5): the trained file had 7 media rows in 5,336 (0.13%), and 6 of the 13 authored media episodes were killed by the exam-leak gate because they REUSED the exam's exact user prompts ("Draw me a picture of a mountain lake at dawn" IS img_basic_en). Plan: ~60 leak-free media episodes (generate_image ×18, video ×12 incl. two-turn consent pairs, read/edit_image, error-recovery), a builder HARD GATE with per-tool floors (build fails below: gen_img ≥12, gen_vid ≥8, edit ≥6, read ≥6, emoji ≥4), offline verification (leaks 0, floors met, soup-path replica), then a one-command pipeline relaunch when the owner approves. Nothing launches while checkpoint-2900 is promoted/live.

"""Web mentor B: capture a strong web assistant's answers on this PC
as training examples for the local model.

The mentor web app has no official Windows desktop app, so this uses the WebView2 engine
...
Usage:
  python mentor_b.py "ما الفرق بين الذاكرة RAM و ROM؟ اشرح بثلاث جمل"
  python mentor_b.py --file questions.txt
"""
...
def _teacher_loop(window, questions: list[str], out_dir: Path, timeout: int) -> list[dict[str, object]]:
    ...
    print("opening the web mentor chat...", flush=True)
    ...
    while time.time() < deadline:
        ...
        if state.get("state") == "no_input" and time.time() < deadline - 90:
            print(
                ...
                "inside the the mentor window (email/phone). Waiting...",
                flush=True,
            )
        time.sleep(4)
    if state.get("state") != "ready":
        print(
            "Could not reach a signed-in the web mentor chat (input box not found). "
            f"Page state: {state}",
            file=sys.stderr,
        )
        raise SystemExit("mentor sign-in required — open the mentor chat in the window and log in.")
...
def main() -> None:
    _utf8_stdio()
    parser = argparse.ArgumentParser(description="Ask the web mentor and save the answer.")
    ...
    if not results:
        raise SystemExit("No mentor answers were captured.")

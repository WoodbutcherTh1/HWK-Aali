# آلي (Aali) — من فريق HWK

مساعد ذكاء اصطناعي **يُبنى ويُدرَّب من الصفر بواسطة فريق HWK** — نموذج لغوي
خاص + وكيل كامل بأدوات حقيقية: ملفات، أوامر آمنة، تحكم بالحاسوب، ذاكرة
...
عميل طرفية احترافي بروح أدوات المبرمجين — ملوّن، حيّ، ويعرض أدوات آلي وهو يعمل:
...
- يعرض **آثار الأدوات** لحظة بلحظة (`● write_file(...)` ثم `└─ نتيجة`) أمام عينيك.
- دوال سريعة: `/new` جلسة جديدة · `/open N` استئناف محادثة · `/clear` · `/theme` ثيمات (ذهبي · matrix · ocean) · `/tools` قائمة قدرات آلي الحية · `/multi` لصق نص متعدد الأسطر · `/help`.
- اختصارات الترمينال: TAB يكمل الأوامر · ↑/↓ تنقّل في سجل الأوامر (محفوظ بين الجلسات) · `\` في نهاية السطر يفتح سطراً جديداً، ولصق كتلة كود يفتح الأسطر تلقائياً.
...
- **دخول بنقرة واحدة:** بعد تسجيل الدخول في واجهة الويب، حساب المدير يفتح اللوحة
  بضغطة على شارة «فريق» — يُولَّد رمز ربط صالحاً لمدة دقيقة ويُستهلك مرة واحدة
  فقط، فتبادل اللوحة الرمز من جهة الخادم بجلسة حقيقية؛ توكن الجلسة نفسه لا يظهر
  في أي رابط أو سجل متصفح، فلا حاجة لنسخ المفتاح يدوياً.
...
- **سجل تدقيق الإجراءات**: كل إجراء إداري (إصدار مفتاح، إلغاؤه، حذف حساب) يُسجّل
  مع الفاعل والوقت وIP — يظهر في اللوحة أسفل المفاتيح، ولا يتضمّن محتوى المحادثات.
- في وضع المستخدم الواحد (بدون `AALI_API_KEY`) افتح `/admin?demo=1` للمعاينة.

![دخول الحساب في واجهة آلي](docs/assets/auth_login.png)

![لوحة تحكم المدير](docs/assets/admin_dash.png)

...
## آلي هو موزّع (Provider)، ليس مجرد مساعد محلي

آلي **منتج** يُقدَّم للمستخدمين حول العالم، مثل Claude / DeepSeek / OpenAI:

- **آلي هو الموزّع.** يعتمد على نموذجه الخاص فقط — لا يستدعي مزوّدي
  ذكاء اصطناعي آخرين ولا يستخدم مفاتيحهم في مسار التشغيل. هو الذي
  **يُصدر مفاتيحه الخاصة** للمستخدمين.
- **آلي يعيش على خادم** يتصل به المستخدمون. لا يتدرّب ولا يعمل على أجهزتهم.
- **هذا الجهاز هو محطة التدريب والاختبار** التي تُبنى عليها Aali (النموذج،
  الامتحانات، إعادة SFT) قبل شحنه.
- **مثل أي شركة ذكاء اصطناعي**: لكل مستخدم مفتاح API خاص، ولوحة تحكم
  للمدير بإصدار/إلغاء المفاتيح ومراقبة الاستخدام، وسجل استخدام لكل مفتاح.
- **Replit** كان نقطة انطلاق فقط — ليس جزءًا من آلي.

### كيف يتصل المستخدمون بآلي

| الطريقة | العنوان | ملاحظة |
|---|---|---|
| المحادثة (الواجهة) | `/ui/` | واجهة الويب المبنية مسبقًا |
| API عام | `POST /api/ask` | `X-API-Key: <مفتاح>` (مفتاح مدير أو مفتاح مستخدم مُصدر) |
| متوافق مع OpenAI | `POST /v1/chat/completions` | `Authorization: Bearer <مفتاح>` — يتطلب مفتاحًا حقيقيًا |
| تسجيل ذاتي | `POST /api/register` | يُصدر مفتاحًا جديدًا (محدود لكل IP/يوم) |
| لوحة تحكم المدير | `/admin` | إصدار/إلغاء/إحصاءات، بمفتاح المدير `AALI_API_KEY` |

المفاتيح تُخزَّن **مجمّعة (SHA-256)** — لا يُخزَّن المفتاح الصريح أبدًا،
والمفتاح الرئيسي `AALI_API_KEY` يُقرأ من متغير البيئة ولا يُخزَّن هنا.

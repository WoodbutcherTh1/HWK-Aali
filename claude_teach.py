"""Claude Desktop teacher: ask the installed Claude app questions on this PC
and save its answers as training examples for the local model.
...
Requirements: Claude Desktop installed and signed in once by the user, plus
pywinauto/pyautogui/pyperclip in the venv. This automates a third-party GUI and
can break when Claude updates; keep answers for personal research only.

Usage:
  python claude_teach.py "Explain overfitting in one short Arabic sentence"
  python claude_teach.py --file questions.txt --timeout 300
"""
...
# AUMID of the installed Claude Desktop app (Anthropic.Claude MSIX).
...
def _launch() -> None:
    os.startfile(f"shell:AppsFolder\\{CLAUDE_AUMID}")  # type: ignore[attr-defined]
...
def _find_candidate_windows() -> list[tuple[int, int, int, bool]]:
    """Return [(hwnd, area, iconic, zero_rect)] for Claude main windows."""
    import ctypes
    from ctypes import wintypes
...
def _locate_main_window() -> tuple[int, tuple[int, int, int, int]]:
    """Find, restore, and focus the main Claude window; return (hwnd, box)."""
    candidates = _find_candidate_windows()
    if not candidates:
        raise SystemExit("Claude window not found. Is Claude Desktop running and signed in?")
...
def _ask_once(hwnd: int, box: tuple[int, int, int, int], prompt: str, timeout: int = 240) -> str:
    ...
    # Claude's composer sits at the bottom of the window.
...
def ask(prompt: str, out_dir: Path, timeout: int = 240) -> str:
    ...
    if not _is_running():
        print("Claude is not running; launching it...")
        _launch()
        time.sleep(8)
    hwnd, box = _locate_main_window()
    print(f"found Claude window hwnd={hwnd} box={box}", flush=True)
    try:
...
def main() -> None:
    _utf8_stdio()
    parser = argparse.ArgumentParser(description="Ask Claude Desktop and save the answer.")
    parser.add_argument("prompt", nargs="?", default=None, help="Question to ask Claude.")
